...
export default function HistoryPage() {
  ...
  const [previewEntries, setPreviewEntries] = useState<ExportEntry[]>([]);
  ...
  async function openPreview(type: "pdf" | "excel") {
    ...
    try {
      const enriched = await Promise.all(
        filtered.map(async (e) => {
          try {
            const res = await fetch(`/api/entries/${e.id}/items`);
  ...
  const excelData     = previewType === "excel" ? getExcelPreviewData(previewEntries, period, branchFilter) : null;
  ...
  return (
    <div className="p-6 space-y-5 max-w-5xl" data-testid="page-history">
      ...
      <AnimatePresence>
        {previewType === "pdf" && (
          <PdfPreviewModal
            entries={previewEntries} period={period} branchFilter={branchFilter}
      ...
      {excelData && (
        <ExcelPreviewModal
          ...
          onDownload={() => exportExcel(previewEntries, period, branchFilter)}
          onClose={() => setPreviewType(null)}
        />
      )}
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex gap-2">
          <button
            ...
            disabled={loadingExport !== null || filtered.length === 0}
          ...
          >
            {loadingExport === "pdf" ? <Loader2 className="h-3.5 w-3.5 animate-spin"/> : <FileText className="h-3.5 w-3.5 text-red-500"/>}
            PDF
          </button>
          <button
            ...
            className="flex items-center gap-2 rounded-xl bg-muted px-4 py-2 text-sm font-bold text-foreground hover:bg-muted/70 transition-all disabled:opacity-50"
          >
            {loadingExport === "excel" ? <Loader2 className="h-3.5 w-3.5 animate-spin"/> : <Table2 className="h-3.5 w-3.5 text-emerald-500"/>}
            Excel
          </button>
        </div>
      </div>
